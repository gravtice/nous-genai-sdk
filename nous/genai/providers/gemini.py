...
from .._internal.errors import invalid_request_error, not_supported_error, provider_error, timeout_error
...
def _resolve_json_schema_ref(ref: str, defs: dict[str, Any]) -> dict[str, Any]:
    if not ref.startswith("#/$defs/"):
        raise invalid_request_error("Gemini responseSchema only supports local $defs $ref")
    name = ref[len("#/$defs/") :]
    if not name or "/" in name:
        raise invalid_request_error("Gemini responseSchema only supports simple $defs refs")
...
def _convert_nullable_union(
    *,
    ...
    if not isinstance(options, list) or not options:
        raise invalid_request_error(f"Gemini responseSchema {tag} must be a non-empty array")
    ...
    for item in options:
        ...
        if not isinstance(item, dict):
            raise invalid_request_error(f"Gemini responseSchema {tag} items must be objects")
...
def _json_schema_to_gemini_schema(
    ...
    *,
    ...
    if not isinstance(schema, dict):
        raise invalid_request_error("output.text.json_schema must be an object for Gemini")
    ...
    if ref is not None:
        ...
        if ref in ref_stack:
            raise invalid_request_error("Gemini responseSchema does not support recursive $ref")
        resolved = _resolve_json_schema_ref(ref, defs)
        out = _json_schema_to_gemini_schema(resolved, defs=defs, ref_stack=ref_stack + (ref,))
    ...
    if all_of is not None:
        if isinstance(all_of, list) and len(all_of) == 1:
            out = _json_schema_to_gemini_schema(all_of[0], defs=defs, ref_stack=ref_stack)
            desc = schema.get("description")
            if isinstance(desc, str) and desc.strip() and "description" not in out:
                out["description"] = desc.strip()
            return out
        raise invalid_request_error("Gemini responseSchema does not support allOf (except a single item)")

    any_of = schema.get("anyOf")
    if any_of is not None:
        out = _convert_nullable_union(tag="anyOf", options=any_of, defs=defs, ref_stack=ref_stack)
        desc = schema.get("description")
        if isinstance(desc, str) and desc.strip() and "description" not in out:
            out["description"] = desc.strip()
        return out
    ...
    if one_of is not None:
        out = _convert_nullable_union(tag="oneOf", options=one_of, defs=defs, ref_stack=ref_stack)
        desc = schema.get("description")
        if isinstance(desc, str) and desc.strip() and "description" not in out:
            out["description"] = desc.strip()
        return out
    ...
    if isinstance(t, list):
        ...
        if "null" in types:
            ...
            if len(types) != 1:
                raise invalid_request_error("Gemini responseSchema only supports nullable union with one non-null type")
            t = types[0]
            nullable = True
        elif len(types) == 1:
            t = types[0]
        else:
            raise invalid_request_error("Gemini responseSchema does not support union types")
    ...
    out: dict[str, Any] = {"type": gemini_type}
    if nullable:
        out["nullable"] = True
    ...
    if gemini_type == "OBJECT":
        ...
        if isinstance(props, dict):
            ...
            for k, v in props.items():
                if not isinstance(k, str) or not k:
                    continue
                out_props[k] = _json_schema_to_gemini_schema(v, defs=defs, ref_stack=ref_stack)
            if out_props:
        ...
        if isinstance(addl, dict):
            out["additionalProperties"] = _json_schema_to_gemini_schema(addl, defs=defs, ref_stack=ref_stack)
    ...
    if gemini_type == "ARRAY":
        ...
        if isinstance(items, dict):
            out["items"] = _json_schema_to_gemini_schema(items, defs=defs, ref_stack=ref_stack)
        elif items is not None:
            raise invalid_request_error("Gemini responseSchema array items must be an object")
...
class GeminiAdapter:
    ...
    def generate(self, request: GenerateRequest, *, stream: bool) -> GenerateResponse | Iterator[GenerateEvent]:
        ...
        if "video" in modalities:
            if modalities != {"video"}:
                raise not_supported_error("video cannot be combined with other output modalities")
            if stream:
        ...
        if stream and (modalities & {"image", "audio"}):
            raise not_supported_error("streaming image/audio output is not supported in this SDK yet")

        if stream:
            return self._generate_stream(request, model_name=model_name)
        return self._generate(request, model_name=model_name)

    def _generate(self, request: GenerateRequest, *, model_name: str) -> GenerateResponse:
    ...
    def _video(self, request: GenerateRequest, *, model_name: str) -> GenerateResponse:
        if self.provider_name.startswith("tuzi") and model_name.startswith("models/veo2"):
            return self._tuzi_veo2_video(request, model_name=model_name)

        if not model_name.startswith("models/veo-"):
            raise not_supported_error('video generation requires model like "google:veo-3.1-generate-preview"')
        ...
        if video and video.duration_sec is not None:
            ...
            if duration < 5 or duration > 8:
                raise invalid_request_error("google veo duration_sec must be between 5 and 8 seconds")
        ...
        if isinstance(opts, dict):
            ...
            if opt_params is not None:
                if not isinstance(opt_params, dict):
                    raise invalid_request_error("provider_options.google.parameters must be an object")
        ...
        if scheme in {"http", "https"}:
            source = PartSourceUrl(url=video_uri)
        else:
            source = PartSourceRef(provider=self.provider_name, id=video_uri)
    ...
    def _tuzi_veo2_video(self, request: GenerateRequest, *, model_name: str) -> GenerateResponse:
        texts = _gather_text_inputs(request)
        if len(texts) != 1:
            raise invalid_request_error("video generation requires exactly one text part")
        prompt = texts[0]
        ...
        if mp4_url:
            part = Part(type="video", mime_type="video/mp4", source=PartSourceUrl(url=mp4_url))
            return GenerateResponse(
                id=f"sdk_{uuid4().hex}",
                provider=self.provider_name,
                model=f"{self.provider_name}:{model_name}",
        ...
        if task_id and request.wait:
            deadline = time.time() + max(1, budget_ms) / 1000.0
            mp4_url = _poll_tuzi_video_mp4(task_id=task_id, deadline=deadline, proxy_url=self.proxy_url)
            if mp4_url:
                part = Part(type="video", mime_type="video/mp4", source=PartSourceUrl(url=mp4_url))
                return GenerateResponse(
                    id=f"sdk_{uuid4().hex}",
    ...
    def _generate_stream(self, request: GenerateRequest, *, model_name: str) -> Iterator[GenerateEvent]:
    ...
    def _embed(self, request: GenerateRequest, *, model_name: str) -> GenerateResponse:
        ...
        if model_name == "models/embedding-gecko-001":
            if dims is not None:
                raise invalid_request_error("models/embedding-gecko-001 does not support embedding.dimensions")
            if len(texts) != 1:
                raise invalid_request_error("models/embedding-gecko-001 only supports single text per request")
            ...
            if not isinstance(values, list) or not all(isinstance(x, (int, float)) for x in values):
                raise provider_error("gemini embedText response missing embedding.value")
            ...
            if not isinstance(embeddings, list) or len(embeddings) != len(texts):
                raise provider_error("gemini batchEmbedContents response missing embeddings")
            parts = []
            for emb in embeddings:
                ...
                parts.append(Part(type="embedding", embedding=[float(x) for x in values]))
        return GenerateResponse(
            id=f"sdk_{uuid4().hex}",
            provider=self.provider_name,
            model=f"{self.provider_name}:{model_name}",
    ...
    def _generate_body(self, request: GenerateRequest, *, model_name: str) -> dict[str, Any]:
        ...
        max_out = text_spec.max_output_tokens if text_spec and text_spec.max_output_tokens is not None else params.max_output_tokens
        ...
        if text_spec and (text_spec.format != "text" or text_spec.json_schema is not None):
            ...
            if text_spec.json_schema is not None:
                gen_cfg["responseSchema"] = _to_gemini_response_schema(text_spec.json_schema)
        ...
        if "audio" in request.output.modalities:
            ...
            if audio is None or not audio.voice:
                raise invalid_request_error("output.audio.voice required for Gemini audio output")
        ...
        if isinstance(opts, dict):
            ...
            if opt_gen_cfg is not None:
                if not isinstance(opt_gen_cfg, dict):
                    raise invalid_request_error("provider_options.google.generationConfig must be an object")
                for k, v in opt_gen_cfg.items():
                    if k in gen_cfg:
                        raise invalid_request_error(f"provider_options cannot override generationConfig.{k}")
                    gen_cfg[k] = v

        body: dict[str, Any] = {"contents": contents}
        if system_text:
            body["systemInstruction"] = {"role": "user", "parts": [{"text": system_text}]}
        if gen_cfg:
        ...
        if request.tools:
            ...
            for t in request.tools:
                ...
                decl["parameters"] = t.parameters if t.parameters is not None else {"type": "object"}
                decls.append(decl)
    ...
    def _upload_to_file_uri(self, file_path: str, *, mime_type: str, timeout_ms: int | None) -> str:
        if not self.supports_file_upload:
            raise not_supported_error("Gemini file upload is not supported for this provider")
...
def _extract_system_text(request: GenerateRequest) -> str | None:
    ...
    for m in request.input:
        ...
        for p in m.content:
            if p.type != "text":
                raise invalid_request_error("system message only supports text for Gemini")
...
def _gemini_supports_thinking_level(model_name: str) -> bool:
    mid = model_name[len("models/") :] if model_name.startswith("models/") else model_name
    mid = mid.strip().lower()
...
def _gemini_supports_thinking_budget(model_name: str) -> bool:
    mid = model_name[len("models/") :] if model_name.startswith("models/") else model_name
    mid = mid.strip().lower()
    return "gemini-2.5-" in mid or "robotics-er" in mid or "flash-live-native-audio" in mid
...
def _thinking_config(reasoning, *, model_name: str) -> dict[str, Any] | None:
    ...
    if reasoning.effort is not None:
        if _gemini_supports_thinking_level(model_name):
            cfg["thinkingLevel"] = _map_effort_to_thinking_level(reasoning.effort, model_name=model_name)
        elif _gemini_supports_thinking_budget(model_name):
            cfg["thinkingBudget"] = _map_effort_to_thinking_budget(reasoning.effort, model_name=model_name)
    return cfg or None
...
def _gemini_response_modalities(modalities: list[str]) -> list[str]:
    out: list[str] = []
    for m in modalities:
        m = m.lower()
        if m == "text":
            ...
            raise not_supported_error("embedding should use embedContent/batchEmbedContents")
        else:
            raise invalid_request_error(f"unknown modality: {m}")
    return out


def _message_to_content(adapter: GeminiAdapter, message: Message, *, timeout_ms: int | None) -> dict[str, Any]:
    ...
    if message.role == "assistant" and any(p.type == "tool_result" for p in message.content):
    ...
    parts = [_part_to_gemini_part(adapter, p, timeout_ms=timeout_ms) for p in message.content]
...
def _part_to_gemini_part(adapter: GeminiAdapter, part: Part, *, timeout_ms: int | None) -> dict[str, Any]:
    if part.type == "text":
    ...
    if part.type in {"image", "audio", "video"}:
        ...
        if not mime_type:
            raise invalid_request_error(f"{part.type} requires mime_type (or path extension)")
        ...
        if isinstance(source, PartSourceUrl):
            ...
            try:
                return _upload_or_inline(adapter, part, file_path=tmp, mime_type=mime_type, timeout_ms=timeout_ms)
            finally:
                try:
                    os.unlink(tmp)
                except OSError:
        ...
        if isinstance(source, PartSourceBytes) and source.encoding == "base64":
            ...
            if not isinstance(b64, str) or not b64:
                raise invalid_request_error(f"{part.type} base64 data must be non-empty")
        ...
        if len(data) <= 20 * 1024 * 1024 and part.type == "image":
            return {"inlineData": {"mimeType": mime_type, "data": bytes_to_base64(data)}}
    ...
    if part.type == "embedding":
        raise not_supported_error("embedding is not a valid Gemini generateContent input part")
...
def _upload_or_inline(
    ...
    *,
    ...
    file_uri = adapter._upload_to_file_uri(file_path, mime_type=mime_type, timeout_ms=timeout_ms)
...
def _gemini_part_to_parts(part: dict[str, Any], *, provider_name: str) -> list[Part]:
    if "text" in part and isinstance(part["text"], str):
        ...
        if provider_name.startswith("tuzi"):
            urls = [m.group(1).strip() for m in _MD_IMAGE_URL_RE.finditer(text) if m.group(1).strip()]
            if urls:
                out: list[Part] = []
                remaining = _MD_IMAGE_URL_RE.sub("", text).strip()
                if remaining:
                    out.append(Part.from_text(remaining))
                out.extend([Part(type="image", source=PartSourceUrl(url=u)) for u in urls])
                return out
    ...
    if "inlineData" in part and isinstance(part["inlineData"], dict):
        ...
        if mime.startswith("image/"):
            return [Part(type="image", mime_type=mime, source=PartSourceBytes(data=data_b64, encoding="base64"))]
        if mime.startswith("audio/"):
            return [Part(type="audio", mime_type=mime, source=PartSourceBytes(data=data_b64, encoding="base64"))]
        if mime.startswith("video/"):
            return [Part(type="video", mime_type=mime, source=PartSourceBytes(data=data_b64, encoding="base64"))]
        return [Part(type="file", mime_type=mime, source=PartSourceBytes(data=data_b64, encoding="base64"))]
    if "fileData" in part and isinstance(part["fileData"], dict):
        ...
        if mime_s and mime_s.startswith("image/"):
            ...
            kind = "video"
        return [Part(type=kind, mime_type=mime_s, source=PartSourceRef(provider=provider_name, id=uri))]
    return []
...
def _poll_tuzi_video_mp4(*, task_id: str, deadline: float, proxy_url: str | None) -> str | None:
